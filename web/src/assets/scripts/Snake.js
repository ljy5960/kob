import { AcgAameObject } from "./AcGameObject";
import { Cell } from "./Cell";
export class Snake extends AcgAameObject{
  ...
  this.direction=d;
    ...
    if(distance<this.eps){//走到目标点
      ...
      this.next_cell=null;
      this.status="idle";//走完
      if(!this.check_tail_increasing()){//蛇不变长，去蛇尾
      this.cells.pop();}
    }else{
      const move_distance=this.speed*this.timedelta/1000;//每两帧帧走过的距离
      this.cells[0].x+=move_distance*dx/distance;//每帧移动的距离
